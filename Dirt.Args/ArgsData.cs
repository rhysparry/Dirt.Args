...
internal class ArgsData : IArgsData
{
    ...
    public ArgsData(IArgsSource args) : this(args.GetArgs())
    {
    }
    ...
    public ArgsData(IEnumerable<string> args)
    {
        ...
        _lazyValueFlags = new Lazy<IReadOnlyDictionary<string, string>>(() => Flags
            .Values
            .Where(f => f.Values.Count > 0)
            .ToDictionary(f => f.Flag, f => f.Values[0]));

        _lazyMultiValueFlags = new Lazy<IReadOnlyDictionary<string, IReadOnlyList<string>>>(() => Flags
            .Values
            .Where(f => f.Values.Count > 0)
            .ToDictionary(f => f.Flag, f => f.Values));
    }
    ...
    public IReadOnlyDictionary<string, IReadOnlyList<string>> MultiValueFlags => _lazyMultiValueFlags.Value;
